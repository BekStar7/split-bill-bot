...
6. Под итогами — кнопка «Оплатил(а)» на каждого участника: свою кнопку может нажать только тот
   человек, к которому она относится (или инициатор — за любого), а отменить отметку может
   только инициатор (чтобы никто не снял с себя долг сам). Список делится на
   «✅ Оплатили» / «❌ Должны» и обновляется на лету.
7. `/debtors` в чате — напомнит, кто ещё должен, по всем посчитанным чекам в этом чате сразу (не только последнему), и упомянет должников.
